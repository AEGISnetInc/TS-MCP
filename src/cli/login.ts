...
export async function runLoginCli(serverName?: string): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Authentication failed' })) as { error?: string };
      throw new Error(errorData.error || 'Authentication failed');
    }

    const result = await response.json() as { sessionToken: string; expiresAt: string; user: string };
